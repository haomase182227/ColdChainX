...
public class GetDiscrepancyDetailQueryHandler : IRequestHandler<GetDiscrepancyDetailQuery, DiscrepancyDetailsResponse?>
{
    ...
    public async Task<DiscrepancyDetailsResponse?> Handle(GetDiscrepancyDetailQuery request, CancellationToken cancellationToken)
    {
        ...
        return new DiscrepancyDetailsResponse
        {
            ...
            ExpectedLengthCm = order?.LengthCm ?? 0,
            ActualLengthCm = lpn.LengthCm ?? 0,
            ExpectedWidthCm = order?.WidthCm ?? 0,
            ActualWidthCm = lpn.WidthCm ?? 0,
            ExpectedHeightCm = order?.HeightCm ?? 0,
            ActualHeightCm = lpn.HeightCm ?? 0,
            IsQuantityDifferent = order != null && order.Quantity != lpn.Quantity,
            IsWeightDifferent = order != null && Math.Abs(order.ExpectedWeightKg - lpn.ActualWeightKg) > 0.01m,
            IsCbmDifferent = order != null && Math.Abs(order.ExpectedCbm - lpn.ActualCbm) > 0.0001m,
            IsLengthDifferent = order != null && Math.Abs(order.LengthCm - (lpn.LengthCm ?? 0)) > 0.01m,
            IsWidthDifferent = order != null && Math.Abs(order.WidthCm - (lpn.WidthCm ?? 0)) > 0.01m,
            IsHeightDifferent = order != null && Math.Abs(order.HeightCm - (lpn.HeightCm ?? 0)) > 0.01m,
