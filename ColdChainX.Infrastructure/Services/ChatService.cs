            ...
            int pageSize)
        {
            if (!requesterRoles.Any(IsStaffRole))
                return ApiResponse<PagedResult<ChatMessageResponse>>.Failure(
                    "Only Sales, Admin, or WarehouseOperator can view customer conversations");
            ...
            Guid? receiverId)
        {
            ...
            if (isCustomer)
            {
                ...
                if (receiverId.HasValue)
                {
                    ...
                    if (!receiverIsStaff)
                        return ApiResponse<object>.Failure("Customer can only send order chat messages to Sales/Admin/WarehouseOperator users");
                }
            ...
            if (!isStaff)
                return ApiResponse<object>.Failure("Only Customer, Sales, Admin, or WarehouseOperator can use order chat");
            ...
            if (receiverId.HasValue)
            {
                ...
                if (receiverId.Value != customerUserId.Value)
                    return ApiResponse<object>.Failure("Sales/Admin/WarehouseOperator can only send this order chat to the order customer");
            }
