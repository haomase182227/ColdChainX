...
[Migration("20260804000000_RenameWarehouseWorkerToWarehouseOperator")]
public partial class RenameWarehouseWorkerToWarehouseOperator : Migration
{
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        migrationBuilder.Sql(
            ...
            BEGIN
                SELECT role_id INTO warehouse_operator_role_id
                FROM public.roles
                ...
                IF warehouse_operator_role_id IS NULL THEN
                    IF warehouse_worker_role_id IS NOT NULL THEN
                        UPDATE public.roles
                        SET role_name = 'WarehouseOperator',
                            description = 'Warehouse operator'
                        WHERE role_id = warehouse_worker_role_id;

                        warehouse_operator_role_id := warehouse_worker_role_id;
                    ELSE
                        INSERT INTO public.roles (role_name, description)
                        VALUES ('WarehouseOperator', 'Warehouse operator')
                        RETURNING role_id INTO warehouse_operator_role_id;
                    END IF;
                END IF;
                ...
                WHERE role_id IN (
                    ...
                    WHERE lower(role_name) IN ('warehouseworker', 'loader', 'manager')
                      AND role_id <> warehouse_operator_role_id
                );
                ...
                WHERE lower(r.role_name) IN ('warehouseworker', 'loader', 'manager')
                  AND r.role_id <> warehouse_operator_role_id
                ...
                SET role_name = 'WarehouseOperator',
                    description = 'Warehouse operator'
                WHERE role_id = warehouse_operator_role_id;

                INSERT INTO public.role_permissions (role_id, perm_id)
                SELECT warehouse_operator_role_id, p.perm_id
